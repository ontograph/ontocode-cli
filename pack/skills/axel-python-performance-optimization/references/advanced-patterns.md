...
In Axel, profile first and treat NumPy/native-extension adoption as an approved dependency and build decision, not a default optimization.
...
@lru_cache(maxsize=None)  # Safe only for a small, closed key domain.
...
# getsizeof reports shallow object overhead; it excludes referenced fields.
regular = RegularClass(1, 2, 3)
slotted = SlottedClass(1, 2, 3)

print(f"Regular object overhead: {sys.getsizeof(regular)} bytes")
print(f"Slotted object overhead: {sys.getsizeof(slotted)} bytes")

# Significant savings with many instances
regular_objects = [RegularClass(i, i+1, i+2) for i in range(10000)]
slotted_objects = [SlottedClass(i, i+1, i+2) for i in range(10000)]

print("Use tracemalloc to compare aggregate memory across the two populations.")
```
