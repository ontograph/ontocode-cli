...
# These are container overheads, not total data memory: getsizeof is shallow.
print(f"List container overhead: {sys.getsizeof(list_data)} bytes")
print(f"Generator container overhead: {sys.getsizeof(gen_data)} bytes")

# A generator avoids materializing the sequence; use tracemalloc to measure peak use.
```
