...
`$ONTOCODE_HOME/skills/ontocode-build/SKILL.md`; read it before forming any build or
...
### Release staging ownership and cleanup

Before creating any release asset, the release driver must create one new,
dedicated staging directory for this run, record its canonical absolute path,
write a run marker inside it, and install cleanup immediately. Do not reuse a
pre-existing `ontocode-release-*` directory. The cleanup trap must run on
success, validation failure, interruption, and cancellation.

At the end of the run, remove only that recorded directory after rechecking
that it is still the same canonical non-symlink directory, owned by the current
user, marked for this run, not a Git repository, and not registered by
`git worktree list --porcelain`. Never delete by glob or prefix. Preserve
distribution clones, Git worktrees, and any directory that fails a safety
check; report `blocked: unsafe release staging cleanup` with the exact path.
If diagnostics must survive a failure, copy only the bounded evidence to the
normal artifact/log location before cleanup. A release is not complete while
its disposable staging directory remains on disk.

### 1. Verify assets

Materialize the project-plan release asset directly from the canonical skill;
do not copy either checked-in compatibility template into staging:

```bash
python3 scripts/sync_project_plan_template.py \
  --output <release-assets-dir>/PROJECT_PLAN_AUTHORING_TEMPLATE.md
```
