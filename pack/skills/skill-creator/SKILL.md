...
- "Where should I create this skill? If you do not have a preference, I will place it in `$ONTOCODE_HOME/skills` (or `~/.ontocode/skills` when `ONTOCODE_HOME` is unset) so Codex can discover it automatically."
...
Before running `init_skill.py`, ask where the user wants the skill created. If they do not specify a location, default to `$ONTOCODE_HOME/skills`; when `ONTOCODE_HOME` is unset, fall back to `~/.ontocode/skills` so the skill is auto-discovered.
...
```bash
scripts/init_skill.py my-skill --path "${ONTOCODE_HOME:-$HOME/.ontocode}/skills"
scripts/init_skill.py my-skill --path "${ONTOCODE_HOME:-$HOME/.ontocode}/skills" --resources scripts,references
