...
> Maintenance: the rules below (and the mirrored entries elsewhere in this file) duplicate axel-review-e2e-specs patterns for generation-time convenience. Pattern semantics — IDs, severities, false-positive exclusions — are owned by `skills/axel-review-e2e-specs/references/pattern-reference.md`; on conflict, that file wins.
...
When a forbidden pattern is genuinely unavoidable, add `// JUSTIFIED: <reason>` on the **line immediately above**. This tells the `axel-review-e2e-specs` to skip the hit during grep checks.
