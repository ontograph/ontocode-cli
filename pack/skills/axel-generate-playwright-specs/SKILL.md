...
# axel-generate-playwright-specs
...
Step 6: YAGNI Audit + axel-review-e2e-specs
...
### axel-review-e2e-specs (automatic quality gate)

Invoke the `axel-review-e2e-specs` skill using the `Skill` tool, targeting the generated spec and POM files. (`axel-review-e2e-specs` ships in this same bundle, so it is normally present. If the `Skill` tool cannot invoke it but the bundle files exist on the host — e.g. a Codex install — do **not** downgrade to scanner-only: read `<axel-review-e2e-specs skill-base>/SKILL.md` and run its full Phase 1–2 procedure inline against the generated spec **and** POM paths, preserving the Phase 2 LLM review and the zero-P0 gate. Fall back to a manual P0 pass (always-true/weak assertions, missing `await`, focused tests) **only** when the axel-review-e2e-specs files are absent entirely, and then state the review ran in reduced form. Never silently skip it.)

- **P0 issues found:** fix immediately, re-invoke `axel-review-e2e-specs`. **Max 3
...
```
## axel-generate-playwright-specs — Complete
...
axel-review-e2e-specs: N P0 (fixed), N P1 (listed below)
Tests: N passed
...
```
## axel-generate-playwright-specs — PARTIAL/BLOCKED
...
- Generate specs into `browser/playwright_tests/e2e/` (or `ui-assurance/`) following existing `uc-*` naming and the fixtures in `fixtures.ts`/`fixtures-client.ts`; page helpers live in `lib/` (`cool-page.ts`).
- Live-page exploration target is the local COOL server started by the repo's own runner — treat it as a local/disposable target; never point at external hosts.
- Run generated tests through `browser/playwright_tests/ui-assurance/run-profile.py` or the pwrunner sidecar, not a fresh `npx playwright test` setup.
