...
- The release driver must install a cleanup trap immediately after creating the
  directory and must execute that cleanup before returning from every success,
  validation-failure, interruption, or cancellation path. A successful upload
  is not complete while the current run's temporary directory remains on disk.
- At the end of a run, re-check the recorded canonical path and remove it only
  when the marker, ownership, non-symlink, non-Git-repository, and unregistered
  worktree checks still pass. If any check fails, leave the directory intact and
  report `blocked: unsafe release staging cleanup` with the exact path.
