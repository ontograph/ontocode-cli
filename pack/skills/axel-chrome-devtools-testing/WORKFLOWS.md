...
This document details key workflows for the chrome-devtools-mcp server.

**Version notice:** these upstream workflows were written against
chrome-devtools-mcp v0.5.1. Newer surfaces renamed or merged tools
(`emulate_network`/`emulate_cpu` -> `emulate`; `navigate_page_history` ->
`navigate_page`; page selection uses `pageId`, not `pageIdx`). The
authoritative, surface-verified reference is `TOOLS.md`; map any workflow step
