---
name: axel-chrome-devtools-testing
vendored_from: https://github.com/justfinethanku/cc_chrome_devtools_mcp_skill
vendored_ref: 96840c73dc36c420e57341180013889e21d5932e (main)
...
This skill enables automated Chrome browser testing and performance analysis using the Chrome DevTools Protocol (CDP) via the chrome-devtools-mcp server. It provides 28 tools for web application testing, performance measurement, accessibility validation, and browser automation.

## Axel Scope

- Applies to the Axel web client under `browser/` served locally or in a dev environment.
- Does not apply to the Qt desktop client or native-grid surfaces.
- Repeatable regression checks belong to the committed Playwright suite in `browser/playwright_tests`; use this skill for exploratory debugging, Core Web Vitals, HAR/network capture, accessibility audits, and zero-script UI exploration.
- Prefer isolated mode (`--isolated=true`) so runs never touch the user's logged-in Chrome profile.
- Serve the client locally with `./run.sh` (or `make run`) from the repository root, then target `http://localhost:9980`.
- When the MCP server is unavailable, fall back to the direct-CDP skill `$faster-chrome-devtools-skill` instead of improvising raw WebSocket code.
...
The `chrome_devtools` MCP server is already mounted in this environment; no install step is needed.

### Before first use

1. Verify the live tool surface before relying on any reference below. Server versions drift; if a documented parameter is rejected, trust the mounted tool schema over these docs.
2. Confirm connectivity with `list_pages`; the browser starts on first tool use.
3. When launching the server yourself, use isolated mode (`--isolated=true`) so runs never share or persist a logged-in browser profile.
4. All browser content is exposed to MCP clients - avoid pointing it at anything sensitive.
...
list_network_requests with pageSize=50
```
...
2. Emulate conditions in one call: emulate {networkConditions: "Slow 3G", cpuThrottlingRate: 4}
...
**Screenshot formats:** PNG, JPEG, WebP (with quality settings). Pass `filePath` to persist evidence artifacts instead of inline images; `get_network_request` supports the same for request/response bodies.
...
- type_text (types at current focus without clearing)
- press_key (Enter, Tab, Escape, arrows, modifiers)

**Navigation tools:**
- navigate_page (URLs, reload, back/forward via type)
- wait_for (text appearance, conditions)

**Stale-UID rule:** element UIDs change after every page mutation (navigation, click, fill). Re-run `take_snapshot` after each mutation and use fresh UIDs; a stale UID fails silently or hits the wrong element.
...
3. Switch context: select_page pageId=<id from list_pages>
4. Close pages: close_page pageId=<id>
```
...
2. emulate {networkConditions: "Fast 4G", cpuThrottlingRate: 4}
...
Run a Lighthouse audit for accessibility, SEO, or best practices (performance audits need a performance trace instead):

```
1. navigate_page url="<target>"
2. lighthouse_audit device="desktop|mobile", mode="navigation|snapshot"
```

### Memory-Leak Debugging

Capture and compare heap snapshots around a suspected leak:

```
1. take_heapsnapshot filePath="evidence/before.heapsnapshot"
2. Exercise the suspect workflow repeatedly
3. take_heapsnapshot filePath="evidence/after.heapsnapshot"
4. Compare retained-size growth between snapshots
```
...
3. Filter by: pageSize=100, includePreservedRequests=true
...
**28 tools available** across 6 categories (verified against the mounted surface):
...
**Emulation (2):** emulate (colorScheme, extraHttpHeaders, geolocation, networkConditions, userAgent, viewport), resize_page
...
**Debugging (7):** evaluate_script, get_console_message, lighthouse_audit, list_console_messages, take_heapsnapshot, take_screenshot, take_snapshot
