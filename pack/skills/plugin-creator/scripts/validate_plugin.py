...
def validate_manifest_shape(
    ...
    for field in ("defaultPrompt", "default_prompt"):
        default_prompt = interface.get(field)
        if default_prompt is None:
            continue
        prompts = default_prompt if isinstance(default_prompt, list) else [default_prompt]
        for index, prompt in enumerate(prompts):
            prompt_field = field if not isinstance(default_prompt, list) else f"{field}[{index}]"
            normalized = " ".join(prompt.split()) if isinstance(prompt, str) else ""
            if len(normalized) > MAX_DEFAULT_PROMPT_LEN:
                errors.append(
                    f"plugin.json field `interface.{prompt_field}` must be at most "
                    f"{MAX_DEFAULT_PROMPT_LEN} characters"
                )
