...
Set a stable path to the skill CLI (default `ONTOCODE_HOME` is `~/.ontocode`):

```
export ONTOCODE_HOME="${ONTOCODE_HOME:-$HOME/.ontocode}"
export IMAGE_GEN="$ONTOCODE_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$ONTOCODE_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
