...
Local (Axel) execution paths, in preference order:
- `playwriter` skill (host-installed Playwriter CLI) for ad-hoc visual checks and screenshots.
- Repo-owned Playwright suites under `browser/playwright_tests/` via `browser/playwright_tests/ui-assurance/run-profile.py` profiles (`ui-smoke`, `ui-complete`) — do not spin up a parallel runner.

Axel notes: target is the local COOL/Qt spreadsheet UI, not a deployed site. Skip Core Web Vitals and deploy/staging steps; console errors, network 4xx/5xx, interaction, visual regression, and accessibility phases apply as-is.
