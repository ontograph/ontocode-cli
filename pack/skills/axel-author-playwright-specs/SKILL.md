---
name: axel-author-playwright-specs
description: >-
...
This is the `setup` project pattern: the setup project (or a `globalSetup` file) runs UI login once, and every browser project replays the saved cookies/localStorage via `storageState` in config.
...
- Suite: `browser/playwright_tests/` in `/home/er77/_wrk/axel`; TypeScript; single local COOL/Qt target (no multi-site, no user login flows).
- Run via `browser/playwright_tests/ui-assurance/run-profile.py` (`ui-smoke`, `ui-complete`) or the pwrunner sidecar; never add a competing runner or config.
- Priority fixes when touching specs: replace `waitForTimeout` (~175 occurrences, worst in `lib/cool-snapshot.ts`) with web-first assertions; implement the stub assertions in `lib/cool-page.ts`; upgrade e2e project from `trace: 'off'` to `retain-on-failure`; remove the hardcoded nvm `v25.5.0` PATH in ui-assurance specs.
