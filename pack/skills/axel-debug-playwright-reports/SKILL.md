---
name: axel-debug-playwright-reports
...
- Artifacts live under `browser/playwright_tests/`: `test-results/`, `playwright-report/`, and pwrunner sidecar runs in `.pwrunner-runs/` (NDJSON via `PWRUNNER_NDJSON_OUT`, JSON via `PLAYWRIGHT_JSON_OUTPUT_NAME`).
- e2e project currently runs `trace: 'off'` — expect no trace.zip for e2e failures until that is fixed; ui-assurance uses `retain-on-failure`.
- Re-run narrowest repro through `browser/playwright_tests/ui-assurance/run-profile.py` or the sidecar, with `--retries=0`.
- For a retry-only or intermittent failure, check prior instability before blaming the change: `python3 scripts/uiux/assurance.py flake-history browser/playwright_tests/.pwrunner-runs`. A test with existing flake history is a test defect until proven otherwise.
- For first-divergence triage across Qt/browser layers, hand off to the `axel-debug-uiux-tests` skill; this skill owns only the Playwright-report side.

## Routine Tool Ownership

This skill owns routine-tool operation 37, `PLAYWRIGHT_FAILURE_TRIAGER`, in
`~/.ontocode/skills/ontocode-routine-tools/references/tool-catalog.md`. Extract
the first divergent boundary, locator/report evidence, console/network signals,
and retry history into the coordinator's shared envelope.
