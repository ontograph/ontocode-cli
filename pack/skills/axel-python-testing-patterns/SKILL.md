---
name: axel-python-testing-patterns
...
- Target Python 3.11+ unless an approved repository compatibility matrix says otherwise.
- Prefer meaningful branch/error-path coverage; use the repository's configured coverage gate when present.
- Keep production code outside test modules and import it normally.
- Isolate environment-dependent tests with monkeypatch or fixtures.
- Register custom markers and enable `--strict-markers`.
- Use optional test dependencies only when they are already approved by the repository's dependency workflow.
...
# Register custom markers in pytest configuration and run with --strict-markers.
@pytest.mark.slow
