...
description: >
  ...
  Vendored base standard for the axel monorepo; pair with axel-rust-supplement
  for FFI, Pin, panic strategy, and supply-chain rules. Invoke with
  /axel-rust-standard.
license: MIT
metadata:
  author: leonardomso
  version: "1.5.1"
  vendored_from: https://github.com/leonardomso/rust-skills
  vendored_ref: fd2a861ab0406a4ac536a55274d14ea6fd1ca9c9
...
This is vendored upstream text. Two workspace rules override it; see
`.claude/skills/axel-rust-supplement/SKILL.md` for the full precedence block.

1. **Edition 2021, MSRV 1.70** (`rust/Cargo.toml`). Rules citing edition 2024 or
   post-1.70 APIs do not apply: `unsafe extern {}`, `#[unsafe(no_mangle)]`,
   `async-fn-in-trait`, `async-async-fn-bounds`, `pat-if-let-chains`.
2. **Measure first.** `mem-smallvec`, `mem-thinvec`, `mem-arrayvec`,
   `mem-compact-string`, `perf-ahash`, and every `opt-*` rule require a profile
   before adoption.

This standard has no FFI, `Pin`, panic-strategy, or supply-chain rules. Those
live in `axel-rust-supplement`, which wins on conflict.
