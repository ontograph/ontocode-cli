...
`<skill-base>` is the directory that contains this SKILL.md — on Claude Code the Skill tool's "Base directory" output (`~/.claude/skills/axel-review-e2e-specs/`), on Codex or the `skills` CLI `~/.agents/skills/axel-review-e2e-specs/`. Auto-detect `<test-dir>` from project structure (common: `e2e/`, `tests/`, `__tests__/`, `spec/`, `cypress/e2e/`).
...
- Review scope: `browser/playwright_tests/` (Playwright only; no Cypress in this repo).
- Known open findings to check first: vacuous `waitForTimeout(0)` stub assertions in `lib/cool-page.ts` used as sole oracle in `e2e/uc-key-02` specs (P0 silent-pass), ~175 `waitForTimeout` calls (worst: 600ms/cell in `lib/cool-snapshot.ts`), `trace: 'off'` on the e2e project in `playwright.config.ts`.
