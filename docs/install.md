...
Pin the current release explicitly with `--release 0.4.2.9`.

Release identity note: GitHub tags, installer inputs, and asset names use the
human release identity such as `0.4.2.9`, while the CLI reports the
machine-readable Cargo version such as `0.4.2+9`.

### Install the optional content pack

Content Pack v8 includes 25 agents and 28 skills. Install it into Ontocode home
with:
...
```bash
curl -fsSL https://raw.githubusercontent.com/ontograph/ontocode-cli/main/scripts/install/install-content-pack.sh | sh -s -- --release content-pack-v8 --scope project --directory /path/to/repo
```
